...
WinJS.Namespace.define("MS.Entertainment.UI.Controls", {NowPlayingPlaylist: MS.Entertainment.UI.Framework.defineUserControl("Controls/NowPlaying/NowPlayingPlaylist.html#nowPlayingPlaylistTemplate", function nowPlayingPlaylistConstructor(element, options) {
        ...
        _bindingsToDetach: null, _eventHandlers: null, _hasSetFocus: false, _uiStateBindings: null, _selectionEvents: null, _invocationHelperEvents: null, galleryTemplate: null, jumpToCurrentIndex: true, initialize: function initialize() {
                ...
                if (this.galleryView && this.galleryTemplate) {
                    ...
                    if (this.galleryView.invocationHelper)
                        this._invocationHelperEvents = MS.Entertainment.Utilities.addEventHandlers(this.galleryView.invocationHelper, {invokedItemChanged: this._invokeItemChanged.bind(this)});
                    this._selectionEvents = MS.Entertainment.Utilities.addEventHandlers(this.galleryView, {selectedIndicesChanged: this._selectionIndicesChanged.bind(this)})
                }
                ...
                this._eventHandlers = MS.Entertainment.Utilities.addEventHandlers(this.domElement, {
                    MSPointerDown: this.galleryMouseDown, keydown: this._keyDown.bind(this)
                });
            ...
            }, _invokeItemChanged: function _invokeItemChanged(args) {
            ...
            }, _selectionIndicesChanged: function _selectionIndicesChanged(args) {
                if (args && args.detail && args.detail.newValue && args.detail.newValue.length)
                    MS.Entertainment.UI.Controls.MusicVisualization.freezeShapes();
                else
                    MS.Entertainment.UI.Controls.MusicVisualization.thawShapes()
                ...
                if (smartDJSeed) {
                    WinJS.Utilities.removeClass(this._header, "removeFromDisplay");
                    WinJS.Utilities.removeClass(this._smartDJButton, "removeFromDisplay");
                    WinJS.Utilities.addClass(this._root, "header")
                }
                else {
                    WinJS.Utilities.addClass(this._header, "removeFromDisplay");
                    WinJS.Utilities.addClass(this._smartDJButton, "removeFromDisplay");
                    WinJS.Utilities.removeClass(this._root, "header")
                }
