...
(function() {
    ...
    WinJS.Namespace.define("MS.Entertainment.UI.Controls", {
        Dashboard: MS.Entertainment.UI.Framework.defineUserControl(null, function dashboardConstructor(element, options) {
            ...
            preventHideDuringInitialize: true, allowAnimations: false, _data: null, _defaultHub: null, _leftOfHomeContainerWidth: 0, _panelsStillToLoad: null, _keyboardNavigationManager: null, _currentStatus: MS.Entertainment.UI.NetworkStatusService.NetworkStatus.unknown, _isFrozen: false, _networkStatusBinding: null, _handlersToCancel: null, _currentLeftScrollLimit: 0, _panelReadyTimeouts: null, _leftOfHomeOffset: 4000, _leftOfHomeControls: null, _leftOfHomeOnScreenPromise: null, _leftOfHomeOnScreen: true, _scrollLeft: 0, _ready: false, _frozenScrollPosition: null, _itemControlAppBarSelection: null, _adjustScrollLimit: function _adjustScrollLimit() {
                    ...
                    var left = WinJS.Utilities.getRelativeLeft(this._leftOfHome.lastElementChild, this._contentScroller);
                    if (MS.Entertainment.Utilities.getTextDirection() === MS.Entertainment.Utilities.TextDirections.RightToLeft)
                        left = (this._leftOfHomeContainerWidth - WinJS.Utilities.getRelativeLeft(this._leftOfHome.lastElementChild, this.domElement) - this._leftOfHome.lastElementChild.clientWidth);
                    ...
                    this._panelReadyTimeouts[panelItem.moniker] = WinJS.Promise.timeout(MS.Entertainment.UI.Controls.Dashboard.READY_TIMEOUT_MS).then(function() {
                    ...
                    if (!this._data.panelsToComplete.length) {
                        (new Microsoft.Entertainment.Instrumentation.Providers.Shell).traceHubStrip_Load_End("dashboard");
                        var event = document.createEvent("Event");
                        event.initEvent("HubStripLoaded", true, false);
                        this.domElement.dispatchEvent(event);
                        (new Microsoft.Entertainment.Instrumentation.Providers.Shell).traceHubStrip_Ready("dashboard");
                        var event = document.createEvent("Event");
                    ...
                    window.requestAnimationFrame(function() {
                        ...
                        if (this._leftOfHome && this._leftOfHome.children && this._leftOfHome.children.length)
                            Array.prototype.forEach.call(this._leftOfHome.children, function(item) {
                                item.scrollPosition = nextOffSet;
                                nextOffSet += item.clientWidth;
                        ...
                        if (this._homeAndToTheRight && this._homeAndToTheRight.children && this._homeAndToTheRight.children.length)
                            Array.prototype.forEach.call(this._homeAndToTheRight.children, function(item) {
                                item.scrollPosition = nextOffSet;
                                nextOffSet += item.clientWidth;
                                scrollPoints.push(nextOffSet)
                            });
                        var snapList = "snapList(" + this._currentLeftScrollLimit + "px, " + scrollPoints.join("px, ") + "px)";
                    ...
                    var leftOfHome = this._data.leftOfHome.reverse();
                    var panelsToLoad = [];
                    panelsToLoad.push({
                    ...
                    if (this._contentScroller)
                        this._frozenScrollPosition = this._contentScroller.scrollLeft;
                    this._isFrozen = true;
                    ...
                    MS.Entertainment.UI.Controls.assert(defaultHub.index !== -1, "Didn't find the hub in the pages list of hubs");
                    ...
                    this._data = {
                        leftOfHome: leftOfHome, homeAndToTheRight: homeAndToTheRight, panelsToLoad: panelMonikers, panelsToComplete: panelMonikers.concat([]), defaultPanelMoniker: panel.iaNode.moniker
                    };
        ...
        }, null, {READY_TIMEOUT_MS: 3000}), DashboardPanel: MS.Entertainment.UI.Framework.defineUserControl("/Controls/DashboardPanel.html#panelTemplate", function dashboardPanel_Constructor(element, options) {
                ...
                _panelTemplatePromise: null, _dataContext: null, _panelAction: null, _isLoading: true, _isFailed: false, _failedModel: null, _applyPanelInformation: function _applyPanelInformation(panelInformation) {
                        MS.Entertainment.UI.Controls.assert(panelInformation, "Didn't get any panel infomration. Need that to render");
                        var dataContext;
                        var panelAction;
                        var eventProvider = new Microsoft.Entertainment.Instrumentation.Providers.Shell;
                        eventProvider.tracePanel_Load_Start(this.panelInformation.moniker || String.empty);
                        ...
                        (new Microsoft.Entertainment.Instrumentation.Providers.Shell).tracePanel_Ready(this.panelInformation.moniker);
                        ...
                        if (!this._isLoading) {
                            (new Microsoft.Entertainment.Instrumentation.Providers.Shell).tracePanel_Load_Start(this.panelInformation.moniker);
                        ...
                        if (this._panelTitle.action && !this._panelTitle.action.automationId)
