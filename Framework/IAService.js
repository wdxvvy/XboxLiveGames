...
WinJS.Namespace.define("MS.Entertainment.InformationArchitecture", {Node: WinJS.Class.define(function constructNode(title, moniker, panelFragmentUrl, viewabilityByRoles, alwaysPutOnBackStack) {
        this.children = [];
        this.title = title;
...
WinJS.Namespace.define("MS.Entertainment.InformationArchitecture", {IAService: WinJS.Class.define(function constructIAService() {
        var rootNode;
        this._nodesByMoniker = {};
        rootNode = this.createNode("", MS.Entertainment.UI.Monikers.root, null, null, MS.Entertainment.InformationArchitecture.ViewRole.page);
        ...
        Object.defineProperty(this, "rootNode", {
            value: rootNode, writable: true, enumerable: true, configurable: false
        })
    }, {
        _nodesByMoniker: null, _featureEnablementModel: null, rootNode: null, _hasValue: function _hasValue(value) {
...
(function() {
    var appIdHandlers = {};
    WinJS.Namespace.define("MS.Entertainment.InformationArchitecture", {
        addIAForAppId: function addIAForAppId(appId, handler) {
            var handlersForAppId = appIdHandlers[appId];
            if (!handlersForAppId) {
                handlersForAppId = [];
                appIdHandlers[appId] = handlersForAppId
            }
            handlersForAppId.push(handler)
        }, getIAHandlersForAppId: function getIAHandlersForAppId(appId) {
                return function callIACreationHandlers(iaService) {
                        var handlers = appIdHandlers[appId];
                        if (!handlers)
                            return;
                        handlers.forEach(function(handlerFunction) {
                            handlerFunction(iaService)
                        })
                    }
            }
    })
