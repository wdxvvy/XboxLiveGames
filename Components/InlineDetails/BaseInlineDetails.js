...
(function(undefined) {
    ...
    WinJS.Namespace.define("MS.Entertainment.Pages", {BaseInlineDetails: MS.Entertainment.UI.Framework.defineUserControl(null, function baseInlineDetails(element, options) {
            ...
            inlineExtraData: null, allowShare: true, _shareOperation: null, _overlay: null, _listLoadingControlPromise: null, _listBindings: null, _disableActionsOnListSelection: false, _errorPanelShown: false, controlName: "BaseInlineDetails", LOADING_PANEL_SHOW_DELAY: 500, LIST_LOADING_SHOW_DELAY: 1500, initialize: function initialize() {
                            ...
                            if (!this.isFailed) {
                                ...
                                if (this.loadingContainer)
                                    MS.Entertainment.Utilities.hideElement(this.loadingContainer).then(function showContent() {
                                        return MS.Entertainment.Utilities.showElement(this.fragmentContainer)
                                    }.bind(this)).then(this._raiseContentReady.bind(this));
                                else
                                    MS.Entertainment.Utilities.showElement(this.fragmentContainer).then(this._raiseContentReady.bind(this)).done(function() {
                    ...
                    if (this.listLoadingContainer)
                        this._listLoadingControlPromise = WinJS.Promise.timeout(delay ? delay : this.LIST_LOADING_SHOW_DELAY).then(function showProgressRing() {
                            ...
                            ring.className = "win-medium win-ring";
    ...
    WinJS.Namespace.define("MS.Entertainment.Pages", {BaseMediaInlineDetails: MS.Entertainment.UI.Framework.deriveUserControl(MS.Entertainment.Pages.BaseInlineDetails, null, function baseInlineDetails(element, options) {
            ...
            controlName: "BaseMediaInlineDetails", blockErrorPanel: false, _mediaContext: null, _appBarService: null, _appbarEventHandlers: null, _isOnline: false, initialize: function initialize() {
                    ...
                    if (lists.length || !this.smartBuyStateEngine) {
                        ...
                        mediaContext.collectionFilter = this.collectionFilter || Microsoft.Entertainment.Queries.MediaAvailability.available
                    }
                    ...
                    switch (this.collectionFilter) {
                        case Microsoft.Entertainment.Queries.MediaAvailability.availableOffline:
                            stringId = String.id.IDS_POPUP_FILTER_AVAILABLE_OFFLINE;
                            break;
                        case Microsoft.Entertainment.Queries.MediaAvailability.availableFromCloud:
                            stringId = String.id.IDS_POPUP_FILTER_IN_CLOUD;
                            break;
                        default:
                            break
                    }
                    ...
                    if (this._showErrorPanel) {
                        ...
                        this._showPanel()
                }, _formatStreamingStatusText: function _formatStreamingStatusText() {
                    var formatStreamingStatusText = function formatStreamingStatusTextFn() {
                            var formatter;
                            var totalTracksFormatted;
                            var streamingTracksFormatted;
                            this.showStreamingStatus = this.media.canDownload;
                            if (this.media.canDownload)
                                if (this.media.LocalTracksCount === 0)
                                    this.streamingStatusText = String.load(String.id.IDS_DETAILS_STREAMING_STATUS_ALL);
                                else {
                                    formatter = MS.Entertainment.ServiceLocator.getService(MS.Entertainment.Services.dateTimeFormatters).decimalNumber;
                                    totalTracksFormatted = formatter.format(this.media.TotalTracksCount);
                                    streamingTracksFormatted = formatter.format(this.media.TotalTracksCount - this.media.LocalTracksCount);
                                    this.streamingStatusText = String.load(String.id.IDS_DETAILS_STREAMING_STATUS_FORMAT).format(streamingTracksFormatted, totalTracksFormatted)
                                }
                            else
                                this.streamingStatusText = null
                        }.bind(this);
                    var binding = WinJS.Binding.bind(this.media, {
                            canDownload: formatStreamingStatusText, LocalTracksCount: formatStreamingStatusText, TotalTracksCount: formatStreamingStatusText
                        });
                    this.mediaBindings.push(binding)
                }
        }, {
            originalLocation: null, detailString: null, smartBuyStateEngine: null, showStreamingStatus: false, streamingStatusText: null, collectionFilter: null, collectionFilterText: String.empty
                ...
                collection: "collection", marketplace: "marketplace"
            }})})
})()
